...
def query_video_resolution(uri):
    """Query video resolution using GStreamer discoverer

    Args:
        uri: Video URI (file:// or rtsp://)
...
def get_optimal_streammux_resolution(uris):
    """Get optimal streammux resolution for all input sources

    Args:
        uris: List of video URIs

    Returns:
        tuple: (width, height) for streammux
    """
    resolutions = []
